...
def check_gradient_linear(linear, h=0.00001):
    ...
    old_val = np.sum(np.square(linear.params['W'])) + np.sum(np.square(linear.params['b']))
    new_val = np.sum(np.square(new_w)) + np.sum(np.square(new_b))
    indirect_loss = linear.alpha * (new_val - old_val)
    direct_loss = np.sum(h * (linear.grads['W_reg'] + linear.grads['b_reg']))
